...
os.chdir('/Users/kevinphan/Desktop/InterviewExam-master 3/ImageClassification') #Change directory to the image classification folder

#First, since the photos are not labelled, I am going to label them externally in order to create a one hot encoding environemnt. 
#The data comes in 11 folders which each has images of each letter.Loop through the folders in our directory and rename accordingly. 
#(A.1, B.5 so on and so forth) In folder 0 is A in folder 1 its B etc.
...
os.chdir('/Users/kevinphan/Desktop/InterviewExam-master 3') #Change back to the exam folder to place the train folder 
...
os.chdir('/Users/kevinphan/Desktop/InterviewExam-master 3/ImageClassification') #Change back to image classification to grab all the photos
#And place them into the train folder outside. Train cant be in the image classfication folder since there will be duplicates.

#I am no taking all the photos and placing it into this folder and this will be our training directory for the neural net. 
#This is just a faster way of doing the manual work.
...
TRAIN_DIR = '/Users/kevinphan/Desktop/InterviewExam-master 3/TRAIN_' #Setting our trainin directory
...
#training data. According to our label which is defined by splitting the image name to isolate the letter, it will return the array with 
#a one for the label that is has.
...
#resizing the arrays in order to feed them into the neural network. Here, -1 refers to the amount of files (batch size), the dimmensions 
#and the last one refers to it being a gray scale image since all images are not coloured. This is where i inspected it the most for my error.
...
conNN = input_data(shape=[None, IMAGE_SIZE, IMAGE_SIZE, 1], name='input')  #This is basically our placeholder function. Determining batch, 
#dimmensions and channel.
...
#to improve the code by debuging the error. I defnitely think it is a broadcasting issue and is probably a small fix. Could also be a 
#TFLearn bug.
